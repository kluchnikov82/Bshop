...
export class ConfirmOrderComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    let localPromo = localStorage.getItem('bshop_promo');
    ...
    regionControl.valueChanges.pipe(debounceTime(300), takeWhile(() => this.isActiveComponent)).subscribe((res) => {
      if (res) {
        if (res.length > 2) {
          ...
          this.eraseAddressControls(['cityControl', 'streetControl', 'flatControl']);
          this.cityOptions = [];
          this.streetOptions = [];
          // this.houseOptions = [];
        }
    ...
    cityControl.valueChanges.pipe(debounceTime(300), takeWhile(() => this.isActiveComponent)).subscribe((res) => {
      if (res) {
        ...
        if (res.length > 1) {
          ...
          this.eraseAddressControls(['streetControl', 'flatControl']);
          this.streetOptions = [];
          // this.houseOptions = [];
        }
      }
  ...
  setUserData() {
    ...
    let localOrderS = localStorage.getItem('bshop_order');
    let fioControl = this.userDataForm.get('fioControl');
    let phoneControl = this.userDataForm.get('phoneControl');
    let emailControl = this.userDataForm.get('emailControl');
    if (localOrderS) {
      ...
      this.userDataForm.get('memoControl').setValue(localOrder.memo);
      // let regionControl = this.deliveryForm.get('regionControl');
  ...
  displayStreet(street?: any) {
    if (street) {
      ...
      } else if (street.data.settlement) {
        adr = street.data.street_with_type + '(' + street.data.settlement_with_type + ')';
      } else {
        adr = street.data.street_with_type;
      }
      let house;
      if (street.data.house) {
        house = street.data.house_type + ' ' + street.data.house;
      }
      if (house) {
        if (street.data.block) {
          house += street.data.block_type + ' ' + street.data.block;
        }
      }
      return adr + ((house) ? ', ' + house : '');
    } else {
      return '';
    }
  }
  ...
  saveOrder() {
    ...
    if (!this.selectedAddress) {
      ...
      if ((this.deliveryForm.valid && this.selectedSuggestion && this.postcode && (this.curShippingMethod != 7) && (this.curShippingMethod != 3))) {
        ...
        if (!this.selectedSuggestion.house) {
          this.errorClass.house = true;
          return;
        }
        address = {
          ...
          house: this.selectedSuggestion.house + (this.selectedSuggestion.block ? (' ' + this.selectedSuggestion.block_type + ' ' + this.selectedSuggestion.block) : ''),
    ...
    if (this.userDataForm.invalid || (this.deliveryForm.invalid && !this.selectedAddress && (this.curShippingMethod != 7))) {
        ...
        if (product.product_id) {
          ...
          if (product.isKit) {
            ...
            kitInfo = {
              ...
              amount: Math.round(price * product.quantity)
            };
            if (product.id) {
      ...
      let surname: string, name: string, patronymic: string, deliveryPointId: any, deliveryPointAdr: string;
      if (fioControl.valid) {
        ...
        if (this.curShippingMethod != 3) {
          ...
          let findPoint = this.dpList.find(item => item.code == this.selectedDP);
          if (findPoint) {
            deliveryPointAdr = this.selectedSuggestion.city_with_type + ', ' + findPoint.address;
          }
        }
        let totalAmount = (this.promoOrder) ? +((this.getCartSum(true))).toFixed(2) : this.getCartSum();
        let totalAmountWD = this.getCartSum();
        let order_info = {
          total_amount: Math.round(totalAmount),
          total_amount_wo_discount: Math.round(totalAmountWD),
          own: this.ownOrder,
          ...
          delivery_point_address: deliveryPointAdr,
