...
export class PopupadviceComponent implements OnInit {
  ...
  sendMessage(partner: boolean) {
    let type;
    if (partner) {
      type = this.typesFeedback.find(i => i.type_name == 'Заявка стать представителем').id;
    } else {
      type = this.typesFeedback.find(i => i.type_name == 'Консультация').id;
    }
    let email = (this.useremail) ? this.useremail : 'info@dari-cosmetics.ru';
    let txt = (partner) ? 'Заявка стать представителем (' + new Date() + ')' : 'Запрос консультации (' + new Date() + ')';
