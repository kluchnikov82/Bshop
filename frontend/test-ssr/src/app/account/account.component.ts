...
export class AccountComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.title.setTitle('Личный кабинет | DARI-COSMETICS');
    ...
    if (AppDataService.user) {
      ...
      if (isPlatformBrowser(this.platformId)) {
        localToken = localStorage.getItem('bshop_t');
        localUserId = localStorage.getItem('bshop_id');
      }
  ...
  getUserData(type) {
    if (this.user) {
      switch (type) {
        ...
        case 'avatar': return (this.user.avatar) ? this.user.avatar : 'https://dari-cosmetics.ru/assets/no-photo.jpg';
      }
  ...
  getUserImg() {
    let src = 'https://dari-cosmetics.ru/assets/feedback.jpg';
    if (this.user) {
      if (this.user.avatar) {
