  ...
  getData() {
    ...
    if (this.id) {
      this.getDataService.getOrderData(this.id).subscribe((data) => {
        ...
        this.orderData = data;
        if (data) {
          this.title.setTitle('Заказ ' + data.order_no + ' | DARI-COSMETICS');
    ...
    if (kit) {
      ...
      if (kitCart) {
        switch (type) {
          ...
          case 'image': return (kit.kit_images) ? kit.kit_images[0].image : 'https://dari-cosmetics.ru/assets/logo-new.jpg';
        }
      }
    }
  }
