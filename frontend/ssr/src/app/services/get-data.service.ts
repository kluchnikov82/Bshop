...
export class GetDataService {
  ...
  public apiUrl = 'https://dari-cosmetics.ru/api/';
  ...
  getMenu() {
    return this.http.get<any>(this.apiUrl + 'shop/categories?limit=100');
  }
  ...
  getCatalogCategory(slug: string) {
    return this.http.get<any>(this.apiUrl + 'shop/categories/' + slug);
  }

  getCatalogSubcategory(slug: string) {
  ...
  getProductComments(id_product: string) {
  ...
  getAllComments(limit, offset) {
    return this.http.get<any>(this.apiUrl + 'shop/feedbacks', { params: {limit: limit, offset: offset } });
  }
  ...
  getMainProdFeedback() {
    return this.http.get<any>(this.apiUrl + 'shop/prod_feedbacks');
  }

  getMainKitFeedback() {
    return this.http.get<any>(this.apiUrl + 'shop/kit_feedbacks');
  }
  ...
  getReferals(limit: number = 100, offset: number = 0) {
    return this.http.get<any>(this.apiUrl + 'appuser/referrals?limit=' + limit + '&offset=' + offset,
                                { headers: {'Authorization': AppDataService.userToken}});
  }
  ...
  bonusToBalance(sum: number) {
    return this.http.post<any>(this.apiUrl + 'appuser/bonus_to_balance/', { amount: sum },
                                {headers: {'Authorization': AppDataService.userToken}});
  }

  changeAvatar(filename: string, img: any) {
    return this.http.put<any>(this.apiUrl + 'appuser/avatar_upload/' + filename, img,
                                {headers: {'Authorization': AppDataService.userToken}});
  }
  ...
  getPaymentLink(order_id: string, shipping: number = 0) {
    return this.http.get<any>(this.apiUrl + 'shop/order_payment/' + order_id,
        { params: {
            bonus_amount: '0',
            deposit_amount: '0',
  ...
  sendProductAdvice(id: string, text: string, url: string, rating: number) {
    return this.http.post<any>(this.apiUrl + 'shop/prod_feedback',
                                {
                                  text: text,
                                  product: id,
                                  video_link: url,
  ...
  sendKitAdvice(id: string, text: string, url: string) {
    return this.http.post<any>(this.apiUrl + 'shop/kit_feedback',
                                {
                                  text: text,
                                  kit: id,
                                  video_link: url
