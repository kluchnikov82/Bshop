...
export class CatalogComponent implements OnInit {
  ...
  openCategory(category: Category, subcat: any = false) {
    ...
    if (subcat) {
      this.meta.updateTag({ name: 'description', content: subcat.SEO_description });
      this.title.setTitle(subcat.SEO_title);
      this.bClist.push({
        text: subcat.name,
        link: '/catalog/' + category.slug + '/' + subcat.slug
      });
    } else {
      this.meta.updateTag({ name: 'description', content: category.SEO_description });
      this.title.setTitle(category.SEO_title);
    }
    this.changeCategory();
  ...
  openProduct(prod: Product) {
    let link = '/catalog/' + this.currentCategory.slug + '/';
    if (this.currentSubcat) {
      link += this.currentSubcat.slug + '/';
    } else {
      link += prod.subcat + '/';
    }
    link += prod.slug;
  ...
  addToCart(product: any = false, event: Event) {
